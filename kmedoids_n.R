# ==============================================================================
# kmedoids_n.R — Clustering Normal (K-Medoids)
# ==============================================================================

# ---- Paquetes ----
pkgs <- c("proxy", "dplyr", "readr", "cluster")
...
.pred_kmedoids_csv <- "predictions/pred_KMEDOIDS_N.csv"
...
# K-Medoids (PAM)
# -----------------------------
run_kmedoids_pam <- function(X, k, dist_method = "cosine") {
  Xmat <- as.matrix(X)
  n <- nrow(Xmat)
  
  if (n > MAX_N_FOR_DIST) {
    stop(paste0("n=", n, " supera MAX_N_FOR_DIST=", MAX_N_FOR_DIST, " (pam con dist sería muy pesado)."))
  }
...
run_clustering_row <- function(dataset, target_cardinality, dataset_name) {
  ...
  start_algo <- Sys.time()
  y_pred <- tryCatch(run_kmedoids_pam(X, k, dist_method = DIST_METHOD), error = function(e) NULL)
  end_algo <- Sys.time()
...
for (i in 1:nrow(odatasets_unique)) {
  cat("\n\n--- Executing KMEDOIDS_N for dataset at position:", i, "---\n")
...
if (length(results_clean) > 0) {
  write.table(do.call(rbind, results_clean),
              .pred_kmedoids_csv, sep = ",",
              row.names = FALSE, col.names = TRUE)
  cat("\nKMEDOIDS_N Guardado en:", .pred_kmedoids_csv, "\n")
} else {
  cat("\nKMEDOIDS_N finalizado sin resultados válidos. No se generó CSV.\n")
}